...
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    $query = "SELECT points, age_threshold, part FROM " . $forms[$occupation_id] . " WHERE id = '" . $_POST['section_id'] . "' LIMIT 1";
    $result = mysqli_query($link, $query);
    $row = mysqli_fetch_array($result);
    $points = $row['points'];
    ...
    if (count(json_decode($_POST["remained_doc_files"])) > 0) {
        $allFiles = array_merge($allFiles, json_decode($_POST["remained_doc_files"]));
    }
    ...
    if ($_POST["document_id"] < 0) {
        ...
        $query = "UPDATE documents SET portfolio_id='" . $portfolio_id . "',
                                       occupation_id='" . $occupation_id . "',
                                       section_id='" . $section_id . "',
                                       part='" . $part . "',
                                       points='" . $points . "',
                                       document_type='" . $document_type . "',
                                       date_of_document='" . $mysqldate . "',
                                       files='" . $allFiles . "',
                                       urls='" . $urls . "',
                                       age_threshold='" . $age_threshold . "' WHERE id='" . $document_id . "' AND employee_id='" . intval($_COOKIE['id']) . "'";
        mysqli_query($link, $query) or die('Unable to execute query. ' . mysqli_error($link));
    }
}
